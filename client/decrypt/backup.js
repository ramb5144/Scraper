...
(function() {
    ...
    function buildTextPositionMap() {
        console.log('🔍 buildTextPositionMap: Starting (server-matching extraction)...');

        // DEBUG: Check highlight count at buildTextPositionMap time
        const highlightsAtBuild = document.querySelectorAll('.encrypted-search-highlight').length;
        console.log(`🔍 buildTextPositionMap: ${highlightsAtBuild} highlights present at build time`);
        ...
        console.log(`🔍 buildTextPositionMap: ${positionMap.length} text nodes, ${fullEncryptedText.length} total chars`);
        console.log(`🔍 buildTextPositionMap: ${nodesInMark} nodes were inside <mark> elements`);
    ...
    function extractTextNodesForSearch() {
        ...
        console.log('🔍 extractTextNodesForSearch: Returning', textNodes.length, 'text nodes');
        return textNodes;
    }
    ...
    console.log('%c✅ Decrypt Interceptor Loaded Successfully', 'color: #4CAF50; font-weight: bold; font-size: 14px;');
    console.log('%c📦 Available Functions:', 'color: #2196F3; font-weight: bold;');
    console.log('  • decryptText(encryptedText) - Decrypt encrypted text');
    console.log('  • encryptLazyContent(text) - Encrypt lazy-loaded content');
    console.log('  • searchEncryptedContent(query) - Search within encrypted content');
    console.log('  • showSearchOverlay() - Show search interface');
    console.log('  • hideSearchOverlay() - Hide search interface');
    
    // Debug: Test copy-paste interception setup
    if (document.addEventListener) {
        console.log('%c📋 Copy-Paste Interception:', 'color: #9C27B0; font-weight: bold;', '✅ Active');
    }
    
    // Debug: Check for encrypted content
    setTimeout(() => {
        const encryptedElements = document.querySelectorAll('[style*="font-family"][style*="Encrypted"], span[style*="word-break"]');
        console.log('%c🔍 Encrypted Content Detection:', 'color: #FF9800; font-weight: bold;', {
            'Encrypted elements found': encryptedElements.length,
            'Status': encryptedElements.length > 0 ? '✅ Content encrypted' : '⚠️ No encrypted elements detected'
        });
    }, 2000);
    
    // Debug: Monitor font loading
    if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(() => {
            // Check for encrypted fonts by looking at:
            // 1. Fonts with 'Encrypted' or 'decryption' in name
            // 2. Fonts loaded from proxy-font URLs (our encrypted fonts)
            // 3. Fonts loaded from R2 URLs with 'decryption_' in filename
            ...
            // Check @font-face rules for encrypted font sources
            ...
            styleTags.forEach(style => {
                ...
                // Check for base64 inlined fonts (data:font/woff2;base64,...)
                const base64Matches = css.match(/data:font\/woff2;base64,[A-Za-z0-9+\/=]+/g);
                if (base64Matches) {
                    base64InlinedFonts += base64Matches.length;
                }
                
                // Check for URL-based fonts (proxy-font or decryption_)
                if (css.includes('proxy-font') || css.includes('decryption_') || css.includes('/fonts/decryption_')) {
                    ...
                    if (urlMatches) {
                        urlBasedFonts += urlMatches.length;
                    }
                }
            });
            
            // Check link tags for encrypted fonts
            const fontLinks = document.querySelectorAll('link[rel="preload"][as="font"], link[href*="decryption_"], link[href*="proxy-font"]');
            
            // Determine loading method
            let loadingMethod = '⚠️ Unknown';
            if (base64InlinedFonts > 0) {
                loadingMethod = '✅ Base64 Inlined (instant - loaded with HTML)';
            } else if (urlBasedFonts > 0) {
                loadingMethod = '🌐 URL-Based (network request required)';
            } else if (encryptedFontsByName.length > 0) {
                loadingMethod = '✅ Loaded (method unknown)';
            }
            
            const fontStatus = {
                'Total fonts loaded': allFonts.length,
                ...
                'Status': (base64InlinedFonts > 0 || urlBasedFonts > 0 || encryptedFontsByName.length > 0 || fontLinks.length > 0) 
                    ? '✅ Encrypted fonts detected' 
                    : '⚠️ No encrypted fonts detected'
            };
            
            console.log('%c🔤 Font Loading Complete:', 'color: #00BCD4; font-weight: bold;', fontStatus);
            
            // Show specific message based on font source
            if (base64InlinedFonts > 0) {
                console.log('%c⚡ Fonts are inlined as base64 - loaded instantly with HTML from R2 cache!', 'color: #4CAF50; font-weight: bold;');
            } else if (urlBasedFonts > 0) {
                console.log('%c⏳ Fonts loaded from URLs - separate network requests required', 'color: #FF9800; font-weight: bold;');
            }
            
            // If we found encrypted font URLs but not by name, that's okay - fonts keep original names
            if ((urlBasedFonts > 0 || base64InlinedFonts > 0) && encryptedFontsByName.length === 0) {
                console.log('%c💡 Note: Encrypted fonts are loaded but keep original font-family names', 'color: #607D8B; font-style: italic;');
            }
        });
    }
    
    // Debug: Expose debug helper
    ...
    console.log('%c💡 Debug Helper: Run debugEncryption() for detailed system information', 'color: #607D8B; font-style: italic;');
})();
